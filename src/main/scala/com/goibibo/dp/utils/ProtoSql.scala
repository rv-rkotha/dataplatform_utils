...
object ProtoSQL {
  ...
  def getSchemaForProtoClass(protoClassName:String) = {
    
    val cl = Class.forName(protoClassName)
    val method = cl.getMethod("messageCompanion")
    val compObj = method.invoke(null)
    val castedCompObj = compObj.asInstanceOf[ T forSome { type T <: GeneratedMessageCompanion[T] } ]
    import collection.JavaConverters._
    StructType(castedCompObj.javaDescriptor.getFields.asScala.map(ProtoSQL.structFieldFor))
  }

  def protoDataFrameToDataFrame(
    sparkSession: SparkSession, 
    protoDF: DataFrame,
    protoClassName: String,
    getBinaryProto : (Row => Array[Byte]),
    getOutputRow : ((Row,Row) => Row),
    getSchema: (StructType, StructType) => StructType
    ): DataFrame = {
      
    val rdd = protoDF.rdd.map( r => {
      val cl = Class.forName(protoClassName)
      val method = cl.getMethod("parseFrom", classOf[ Array[Byte] ])
      val binaryProto:Array[Byte] = getBinaryProto(r)
      val protoObject = method.invoke(null, binaryProto).asInstanceOf[T forSome { type T <: GeneratedMessage with Message[T] }]
      val protoRow = messageToRow(protoObject)
      getOutputRow(r, protoRow)
    })
    
    val protoSchema = getSchemaForProtoClass(protoClassName)
    val rddSchema   = getSchema(protoDF.schema, protoSchema)
    
    sparkSession.createDataFrame(rdd, rddSchema)
  }
